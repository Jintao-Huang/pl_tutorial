...
def plot_dists(val_dict, color="C0", xlabel=None, stat="count", use_kde=True):
    ...
    for fig_index, key in enumerate(val_dict.keys()):
        ...
        sns.histplot(
            ...
            # [?]
            kde=use_kde and (
                (val_dict[key].max() - val_dict[key].min()) > 1e-8),
        )
        hidden_dim_str = (
            r"(%d $\to$ %d)" % (val_dict[key].shape[1], val_dict[key].shape[0]) if len(
                val_dict[key].shape) > 1 else ""
        )
    ...
    with torch.no_grad():
        for layer_index, layer in enumerate(model.layers):
            ...
            if isinstance(layer, nn.Linear):
                activations[f"Layer {layer_index}"] = feats.view(
                    -1).cpu().numpy()

    #
    fig = plot_dists(activations, color=color,
                     stat="density", xlabel="Activation vals")
...
def train_model(net, model_name, optim_func, max_epochs=50, batch_size=256, overwrite=False):
    ...
    print(
        (f" Test accuracy: {results['test_acc'] * 100:4.2f}% ").center(50, "=") + "\n")
    return results
...
class SGDMomentum(OptimizerTemplate):
    ...
    def update_param(self, p):
        self.param_momentum[p] = (1 - self.momentum) * \
            p.grad + self.momentum * self.param_momentum[p]
...
class Adam(OptimizerTemplate):
    def __init__(self, params, lr, beta1=0.9, beta2=0.999, eps=1e-8):
        ...
        self.beta2 = beta2
        self.eps = eps
        # "t" for each parameter for bias correction
    ...
    def update_param(self, p):
        ...
        self.param_momentum[p] = (1 - self.beta1) * \
            p.grad + self.beta1 * self.param_momentum[p]
        self.param_2nd_momentum[p] = (
            1 - self.beta2) * p.grad ** 2 + self.beta2 * self.param_2nd_momentum[p]
...
def plot_curve(
    ...
    if plot_3d:
        ...
        ax.imshow(z, cmap=cmap, origin='lower', extent=(
...
def train_curve(optimizer_func, curve_func=pathological_curve_loss, num_updates=100, init=[5, 5]):
    ...
    for _ in range(num_updates):
        ...
        list_points.append(torch.stack(
            [*weights.detach(), loss.detach()], dim=0))
        optim.zero_grad()
        loss.backward()
        optim.step()
...
SGDMom_points = train_curve(
    lambda params: SGDMomentum(params, lr=10, momentum=0.9))
...
ax.plot(SGD_points[:, 0], SGD_points[:, 1], marker="o",
        linestyle='-', color="red", label="SGD")
ax.plot(SGDMom_points[:, 0], SGDMom_points[:, 1],
        marker="o", linestyle='-', color="blue", label="SGDMom")
ax.plot(Adam_points[:, 0], Adam_points[:, 1], marker="o",
        linestyle='-', color="grey", label="Adam")
plt.legend()
plt.show()
...
_ = plot_curve(comb_func, x_range=(-2, 2), y_range=(-2, 2),
               plot_3d=True, title="Steep optima")
plt.show()

SGD_points = train_curve(lambda params: SGD(
    params, lr=0.5), comb_func, init=[0, 0])
SGDMom_points = train_curve(lambda params: SGDMomentum(
    params, lr=1, momentum=0.9), comb_func, init=[0, 0])
Adam_points = train_curve(lambda params: Adam(
    params, lr=0.2), comb_func, init=[0, 0])

all_points = np.concatenate([SGD_points, SGDMom_points, Adam_points], axis=0)
ax = plot_curve(comb_func, x_range=(-2, 2), y_range=(-2, 2),
                plot_3d=False, title="Steep optima")
ax.plot(SGD_points[:, 0], SGD_points[:, 1], marker="o",
        linestyle='-', color="red", label="SGD", alpha=0.2)
ax.plot(SGDMom_points[:, 0], SGDMom_points[:, 1], marker="o",
        linestyle='-', color="blue", label="SGDMom", alpha=0.2)
ax.plot(Adam_points[:, 0], Adam_points[:, 1], marker="o",
        linestyle='-', color="grey", label="Adam", alpha=0.2)
ax.set_xlim(-2, 2)
ax.set_ylim(-2, 2)
plt.legend()
plt.show()
